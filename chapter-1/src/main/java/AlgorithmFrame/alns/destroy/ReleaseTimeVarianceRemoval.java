...
/**
 * RTVR - Release Time Variance Removal
 * 移除释放期方差最大的批次
 */
public class ReleaseTimeVarianceRemoval implements DestroyOperator {
    ...
    public List<Integer> destroy(ALNSSolution solution, ALNSJob[] jobs, ALNSMachine[] machines, int numToRemove) {
        ...
        for (int machineId = 0; machineId < machines.length; machineId++) {
            for (ALNSBatch batch : solution.getMachineBatches(machineId)) {
                ...
                double variance = calculateReleaseTimeVariance(batch, jobs);
    ...
    private double calculateReleaseTimeVariance(ALNSBatch batch, ALNSJob[] jobs) {
        if (batch.jobs.size() <= 1) return 0.0;
        
        double sum = 0.0;
        for (int jobId : batch.jobs) {
            sum += jobs[jobId].releaseTime;
        }
        double mean = sum / batch.jobs.size();
        
        double variance = 0.0;
        for (int jobId : batch.jobs) {
            double diff = jobs[jobId].releaseTime - mean;
            variance += diff * diff;
        }
