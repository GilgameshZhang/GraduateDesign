...
public class Input {
    ...
    public Problem getProblemDesFromFile() {
        ...
        try {
            ...
            //第二行为打印机器数 每个打印机器分别由 name L W H printSpeed recordTime prepareTime
            ...
            input.setBatchMachineCount(batchMachineCount);
            
            // 计算离散加工机数量
            int discreteMachineCount = machineNum - printMachineCount - batchMachineCount;
            System.out.println("离散加工机数：" + discreteMachineCount);
            
            // *** 读取能耗参数（第4-6行） ***
            // 打印机能耗参数
            String printerEnergyLine = reader.readLine();
            if (printerEnergyLine != null && !printerEnergyLine.trim().isEmpty()) {
                // 存储到Problem对象中，供后续使用
            ...
            if (batchEnergyLine != null && !batchEnergyLine.trim().isEmpty()) {
                input.setBatchEnergyParams(batchEnergyLine.trim());
            }
            
            // 离散机能耗参数
            String discreteEnergyLine = reader.readLine();
            if (discreteEnergyLine != null && !discreteEnergyLine.trim().isEmpty()) {
            ...
            for (int i = 0; i < printMachineCount; i++) {
                machines[i] = new PrintMachine(
                    String.valueOf(count1++), 
                    Double.valueOf(printMachineStrArr[count1++]), 
                    Double.valueOf(printMachineStrArr[count1++]), 
                    Double.valueOf(printMachineStrArr[count1++]), 
                    Double.valueOf(printMachineStrArr[count1++]),
                    Double.valueOf(printMachineStrArr[count1++]), 
            ...
            for (int i = printMachineCount; i < printMachineCount + batchMachineCount; i++) {
                machines[i] = new BathchMachine(
                    String.valueOf(count1++), 
                    Double.valueOf(batchMachineStrArrArr[count1++])
                );
            }
            
            // 创建离散加工机对象
            for (int i = printMachineCount + batchMachineCount; i < machineNum; i++) {
                machines[i] = new ProgramEntity.Machine.DiscreteProcessingMachine();
                machines[i].name = String.valueOf(i - printMachineCount - batchMachineCount + 1);
            }
